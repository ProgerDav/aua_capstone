...
def run_evaluation(
    ...
    for item, input_prompt, result in zip(filtered_data, input_list, output_list):
        ...
        metric, pred_answer = evaluate_predictions(
            output=item["Output"], labeled_answer=labeled_answer, mode=mode
        )
        ...
        my_method_valid = pred_answer != "" and not (
            mode == "choose" and dataset_name == "gpqa" and len(pred_answer) > 1
        )
    ...
    metrics_json_name = (
        f"{split}.{t.tm_mon}.{t.tm_mday},{t.tm_hour}:{t.tm_min}.metrics.json"
    )
    ...
    if dataset_name == "gpqa":
        for dm, m in domain_metrics.items():
            domain_avg_metrics[dm] = {
                ...
                "math_equal": (
                    np.mean(m["math_equal"]) if len(m["math_equal"]) > 0 else 0
                ),
    ...
    with open(
        os.path.join(output_dir, metrics_json_name), mode="w", encoding="utf-8"
    ) as json_file:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_metrics_path", type=str, help="Path to save the evaluation metrics."
    )
    parser.add_argument(
        "--apply_backoff",
        action="store_true",
        help="Enable backoff to normal outputs if main output is invalid.",
    )
    ...
    if "gpqa" in output_path:
        ...
        normal_output_path = (
            "./outputs/runs.qa/triviaqa.qwq.direct/test.12.15,15:35.json"
        )
        if "qwq" not in output_path:
            normal_output_path = ""
        ...
        normal_output_path = (
            "./outputs/runs.qa/hotpotqa.qwq.direct/test.12.15,14:52.json"
        )
        if "qwq" not in output_path:
            normal_output_path = ""
        ...
        normal_output_path = (
            "./outputs/runs.qa/musique.qwq.direct/test.12.27,16:44.json"
        )
        if "qwq" not in output_path:
            normal_output_path = ""
        ...
        normal_output_path = (
            "./outputs/runs.qa/bamboogle.qwq.direct/test.12.28,9:51.json"
        )
        if "qwq" not in output_path:
            normal_output_path = ""
        ...
        normal_output_path = (
            "./outputs/runs.qa/medmcqa.qwq.direct/test.12.15,16:57.json"
        )
        if "qwq" not in output_path:
            normal_output_path = ""
        ...
        normal_output_path = (
            "./outputs/runs.qa/pubhealth.qwq.direct/test.12.15,20:32.json"
        )
        if "qwq" not in output_path:
            normal_output_path = ""
    ...
    if args.apply_backoff:
        if not os.path.exists(normal_output_path):
            raise FileNotFoundError(
                f"Normal output file not found at: {normal_output_path}"
            )
    ...
    if dataset_name != "livecode":
        ...
        for i, item in enumerate(data):
            ...
            if args.apply_backoff and not my_method_valid and normal_data is not None:
                ...
                normal_valid = normal_pred_answer != "" and not (
                    normal_mode == "choose"
                    and dataset_name == "gpqa"
                    and len(normal_pred_answer) > 1
                )
