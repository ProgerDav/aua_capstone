...
import torch
from bing_search import (bing_web_search, extract_relevant_info,
                         fetch_page_content)
from evaluate import run_evaluation
from prompts import (get_code_rag_agent_instruction,
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run RAG Agent for various datasets and models."
    )
    ...
    parser.add_argument(
        "--max_turn", type=int, default=10, help="Maximum number of turns."
    )
...
def main():
    ...
    if "qwq" in model_path.lower():
        ...
        output_dir = (
            f"./outputs/runs.baselines/{dataset_name}.{model_short_name}.rag_agent"
        )
    ...
    for item in filtered_data:
        ...
        if dataset_name in [
            ...
            if dataset_name in ["nq", "triviaqa"]:
                instruction = get_singleqa_rag_agent_instruction(
                    MAX_SEARCH_LIMIT, MAX_URL_FETCH
                )
            elif dataset_name in ["hotpotqa", "musique", "bamboogle", "2wiki"]:
                instruction = get_multiqa_rag_agent_instruction(
                    MAX_SEARCH_LIMIT, MAX_URL_FETCH
                )
            if "qwq" in model_path.lower():
            ...
            instruction = get_math_rag_agent_instruction(
                MAX_SEARCH_LIMIT, MAX_URL_FETCH
            )
            if "qwq" in model_path.lower():
            ...
            instruction = get_gpqa_rag_agent_instruction(
                MAX_SEARCH_LIMIT, MAX_URL_FETCH
            )
            if "qwq" in model_path.lower():
            ...
            instruction = get_code_rag_agent_instruction(
                MAX_SEARCH_LIMIT, MAX_URL_FETCH
            )
    ...
    while True:
        ...
        sequences_with_pending_ops = [
            seq
            for seq in active_sequences
            if not seq["finished"] and seq["pending_operations"]
        ]
        sequences_needing_generation = [
            seq
            for seq in active_sequences
            if not seq["finished"] and not seq["pending_operations"]
        ]

        # First, handle pending operations
        if sequences_with_pending_ops:
            print(
                f"{len(sequences_with_pending_ops)} sequences have pending operations. Executing..."
            )
            for seq in sequences_with_pending_ops:
                ...
                if op_type == "search":
                    ...
                    search_result_str = json.dumps(
                        relevant_info, ensure_ascii=False, indent=2
                    )
                    ...
                    urls_to_fetch = [
                        u for u in urls_to_fetch if u not in seq["executed_url_fetches"]
                    ]
                    ...
                    if not urls_to_fetch:
                        print(
                            "All requested URLs have been fetched or exceeded the limit."
                        )
                        continue

                    # Batch fetch page content, considering cache
                    urls_to_fetch_filtered = [
                        u for u in urls_to_fetch if u not in url_cache
                    ]
                    ...
                    full_page_str = json.dumps(
                        fetched_pages, ensure_ascii=False, indent=2
                    )
        ...
        if sequences_needing_generation:
            turn += 1
            print(
                f"Turn {turn}: {len(sequences_needing_generation)} sequences need generation. Generating with LLM..."
            )
            ...
            for seq, out in zip(sequences_needing_generation, outputs):
                ...
                if search_query:
                    ...
                    if seq["search_count"] < MAX_SEARCH_LIMIT:
                        ...
                        if search_query not in seq["executed_search_queries"]:
                            ...
                            seq["pending_operations"].append(
                                {"type": "search", "content": search_query}
                            )
                            seq["executed_search_queries"].add(search_query)
                            print(
                                f"Added pending search operation for query: {search_query}"
                            )
                        else:
                ...
                if url_fetch:
                    ...
                    if len(seq["executed_url_fetches"]) < MAX_URL_FETCH:
                        ...
                        urls_to_fetch = [
                            u for u in urls if u not in seq["executed_url_fetches"]
                        ]
                        if urls_to_fetch:
                            ...
                            seq["pending_operations"].append(
                                {
                                    "type": "fetch_url",
                                    "content": ", ".join(urls_to_fetch),
                                }
                            )
                            print(
                                f"Added pending URL fetch operation for URLs: {urls_to_fetch}"
                            )
                        else:
                            print(
                                f"All requested URLs have been fetched or exceeded the limit: {urls}"
                            )
                    else:
