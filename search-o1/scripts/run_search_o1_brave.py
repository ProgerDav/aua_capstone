...
import numpy as np
import torch
from brave_search import (brave_web_search, extract_relevant_info,
                          extract_snippet_with_context, fetch_page_content)
...
def parse_args():
    ...
    parser.add_argument(
        "--brave_api_key", type=str, required=True, help="Brave Search API key."
    )
...
def main():
    ...
    if "qwq" in model_path.lower():
        ...
        output_dir = (
            f"./outputs/runs.baselines/{dataset_name}.{model_short_name}.search_o1"
        )
        ...
        if sequences_needing_generation:
                ...
                if search_query and seq["output"].rstrip().endswith(END_SEARCH_QUERY):
                    if (
                        ...
                        if search_query in search_cache:
                            ...
                            print(
                                f'Using cached search results for query: "{search_query}"'
                            )
                        else:
                            try:
                                results = brave_web_search(search_query, brave_api_key)
                                search_cache[search_query] = results
                                print(
                                    f'Executed and cached search for query: "{search_query}"'
                                )
                            except Exception as e:
                                print(
                                    f"Error during search query '{search_query}': {e}"
                                )
            ...
            if all_urls_to_fetch:
                ...
                try:
                    ...
                    fetched_contents = {
                        url: f"Error fetching URL: {e}" for url in all_urls_to_fetch
                    }
            ...
            for relevant_info in batch_relevant_info:
                ...
                for i, doc_info in enumerate(relevant_info):
                    ...
                    doc_info["description"] = (
                        doc_info["description"].replace("<b>", "").replace("</b>", "")
                    )
                    success, filtered_context = extract_snippet_with_context(
                        raw_context, doc_info["description"], context_chars=max_doc_len
                    )
                    if success:
