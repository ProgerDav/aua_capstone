...
ENGINE_CONFIG = {
    ...
    "enable_prompt_tokens_details": os.environ.get(
...
@bentoml.service(
    ...
    },  # Relevant for BentoML Cloud deployments
    envs=[
        ...
        # Add this to explicitly set the torch compile cache directory
        {
            ...
            "value": "/home/ubuntu/.cache/vllm/torch_compile_cache",
        },
    ],
...
class VLLM:
    ...
    model = bentoml.models.HuggingFaceModel(
        model_id, exclude=["*.pth", "*.pt", "original/**/*"]
    )
    ...
    async def init_engine(self) -> None:
        ...
        args.tool_call_parser = (
            "llama3_json"  # Unused at the moment, we don't use tools
        )
        ...
        self.engine = await self.exit_stack.enter_async_context(
            vllm_api_server.build_async_engine_client(args)
        )
        self.model_config = await self.engine.get_model_config()
        self.tokenizer = await self.engine.get_tokenizer()
        await vllm_api_server.init_app_state(
            self.engine, self.model_config, openai_api_app.state, args
        )
    ...
    async def generate(
        ...
        max_tokens: typing_extensions.Annotated[
            int,
            annotated_types.Ge(128),
            annotated_types.Le(ENGINE_CONFIG["max_model_len"]),
