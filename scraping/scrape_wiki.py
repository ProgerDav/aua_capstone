        ...
        for attempt in range(max_retries):
            try:
                ...
                result["size_mb"] = round(
                    os.path.getsize(output_file) / (1024 * 1024), 2
                )
                ...
                if attempt < max_retries - 1:
                    ...
                    print(
                        f"Failed to generate PDF for '{title}' from {lang} Wikipedia after {max_retries} attempts: {e}"
                    )
...
def process_wikipedia_titles(titles, max_workers=4, output_dir="./wiki_pdfs"):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        # Submit all tasks
        future_to_title = {
            executor.submit(download_and_update, title): title for title in titles
        }
    ...
    print(
        f"Successfully downloaded {success_count} of {len(titles)} Wikipedia articles as PDFs"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_file", type=str, default="./data/FlashRAG_datasets/2wiki/test.json"
    )
    ...
    df, failed_titles = process_wikipedia_titles(
        titles, max_workers=12, output_dir=args.output_dir
    )
