...
class MMRAG:
    ...
    def vidorag(self, sample):
        ...
        print(query)
        recall_results = self.search_engine.search(query)
        candidate_image = [  # list of image paths retrieved by search engine
            os.path.join(
                self.img_dir,
                os.path.basename(
                    node["node"]["metadata"]
        ...
        sample["recall_results"] = dict(
            ...
            response=None,
            metadata=None,
        )  ## return the nodes chosen by the search engine (not the one chosen by Seeker, Inspector or Synthesizer)
        return sample
        ...
        if self.workers_num == 1:
            for item in tqdm(data):
                ...
                with open(self.output_file_path, "a") as f:
                    ...
                    f.write("\n")
                    # save the information: uid, query, reference_answer, response, recall_results
        else:
...
def arg_parse():
    ...
    parser.add_argument(
        "--dataset", type=str, default="ExampleDataset", help="The name of dataset"
    )
    parser.add_argument(
        "--query_file",
        type=str,
    ...
    parser.add_argument(
        "--generate_vlm", type=str, default="qwen-vl-max", help="The name of VLM model"
    )
...
if __name__ == "__main__":
    ...
    # -------- Example for ExampleDataset with ViDoRAG --------
    mmrag = MMRAG(
        dataset="ExampleDataset",
        query_file="rag_dataset.json",
        ...
        # embed_model_name_text=args.embed_model_name_text,
        generate_vlm="Qwen/Qwen2.5-VL-7B-Instruct",
    )
    mmrag.eval_dataset()
    mmrag.eval_overall()
    mmrag.eval_overall_type_wise()

    # -------- Example for ViDoSeek dataset with ViDoRAG --------
    mmrag = MMRAG(
        dataset="ViDoSeek",
    ...
    # -------- Example for SlideVQA dataset with ViDoRAG --------
    # mmrag = MMRAG(
    #     dataset="ViDoSeek",
    #     query_file="vidoseek.json",
    #     experiment_type="vidorag",
