...
class SearchEngine:
    def __init__(
        ...
        if node_dir_prefix is None:
            ...
            elif "colnomic" in embed_model_name:
                node_dir_prefix = "colnomic_ingestion"
            else:
        ...
        if (
            "vidore" in embed_model_name
            or "openbmb" in embed_model_name
            or "colnomic" in embed_model_name
        ):
            if self.vl_ret:
    ...
    def load_nodes(self):
        ...
        if max_workers == 1:
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                results = list(
                    tqdm(
                        executor.map(parse_file, files, [self.node_dir] * len(files)),
                        total=len(files),
                    )
                )
    ...
    def load_query_engine(self):
        ...
        if self.vl_ret and (
            "vidore" in self.embed_model_name or "colnomic" in self.embed_model_name
        ):
            self.embedding_img = [
    ...
    def search(self, query):
        ...
        if self.gmm:
            recall_results_output = gmm(
                recall_results, self.input_gmm, self.max_output_gmm, self.min_output_gmm
            )
        if self.return_raw:
...
class HybridSearchEngine:
    ...
    def search(self, query):
        ...
        if union_result:
            ...
            overleap = [
                doc for doc in result_docs_vl_list if doc in result_docs_text_list
            ]
            ...
            candidate_overleap = candidate_overleap + [
                ...
                if (node not in candidate_overleap) and (node not in result_docs_list)
            ]
