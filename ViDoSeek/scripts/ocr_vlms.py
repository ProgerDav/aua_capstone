...
def draw_boxes(img_path, boxes):
    ...
    for box in boxes:
        cv2.rectangle(
            img,
            (int(box[1] * x_len / 1000), int(box[0] * y_len / 1000)),
            (int(box[3] * x_len / 1000), int(box[2] * y_len / 1000)),
            (0, 255, 0),
            2,
        )
    # save the image
...
def ocr(img_path):
    while True:
        try:
            ...
            # save json
            with open(
                img_path.replace("img", "vlmocr").replace(".jpg", ".json"), "w"
            ) as f:
...
if __name__ == "__main__":
    ...
    img_files = [
        ...
        if file.replace(".jpg", ".json") not in already_done
    ]
    if workers_num == 1:
        ...
        with ThreadPoolExecutor(max_workers=workers_num) as executor:
            futures = [
                executor.submit(ocr, os.path.join(img_dir, img_file))
